...
THE LIVE ARENA. When your briefing carries a gym section you are on Base, with real inventory (a few tens of dollars per token, declared as your ledger) against the real pools. The gym section is the record of the training seasons on a fork: what each line shipped, generation by generation, and what it attested; the live field started from the last of those. On Base the flow is the arena's own taker: it arbitrages any quote that beats the real pool by more than 5 bps and sends small uninformed orders that shop as before; no taker reads a tape ahead, but the market itself moves, and every fill is re-marked five minutes later at the real pool. Programs ship with real transactions; a program the validator refuses costs nothing.
