...
6. `OraclePriceAdjuster1D` is not an anchor. It runs after the swap, reads a Chainlink-shaped feed and only ever moves the price in the taker's favour (a better fill on a limit order). A market maker needs the opposite, the curve re-centred on the oracle before the swap, both ways; that is a separate instruction. Any contract answering `latestRoundData()` serves as the feed, a Uniswap v3 pool included.
7. A maker's fill is settled with `transferFrom` out of its wallet and the only callbacks are the taker's. No flash liquidity, no maker hook: inventory must sit in the wallet at fill time, one wallet backs every strategy the maker ships, and a strategy quotes only the side it can deliver.
8. `ProgramBuilder` finds opcodes by comparing internal function pointers; the test contract must inherit the opcode table (the upstream tests do the same) and the compiler warns that the comparison is fragile under via-IR. It works on 0.8.30 with the pinned settings; keep the warning in mind when it stops working.
9. On a fork, balances are storage writes: USDC on Base (FiatTokenV2_2) keeps `balanceAndBlacklistStates` in slot 9, WETH9 keeps `balanceOf` in slot 3; `anvil_setStorageAt` at `keccak(abi.encode(account, slot))` funds any account, and forge's `deal` finds the same slots on its own. Whales are not needed.
10. An oracle-anchored maker cannot be picked off against its own oracle: the arbitrage take never fires when the gladiator quotes at the pool it is scored against. Informed flow in the gym must come from the future: the taker reads the tape one step ahead and trades the gladiators before the pool moves.
11. Salt defeats byte-level dedupe: about 84 percent of mainnet programs end in a Salt instruction, so byte-identical dedupe collapses almost nothing. Rule: the template (opcode sequence, args ignored) is the taxonomy; novelty claims are made at template level.
...
- Gating, measured 2026-09-07: the dominant template's gate is `OnlyTxOriginTokenBalanceNonZero` on `0x26ffc7d378e8e49be2c483295a3e3e511f96a468`, "RES", the "Access Token for SwapVM v3.1.2": the tx origin must hold the resolver access token. Gated volume is $826M of $836M on Ethereum, $3.06M of $3.23M on BSC, $44.5K of $44.6K on Base. Outsiders cannot fill 99% of Aqua's flow.
- The ungated remainder is not an opportunity. Live ungated strategies on 2026-09-07: Ethereum 601, BSC 115, Base 17. Quoting every one on a fork against the maker's real wallet balance and allowance (`Aqua.safeBalances` is the raw ledger, it does not check the wallet): 510 of the Ethereum 601 cannot pay at all (phantom ledgers), 33 quote nothing, 54 are fillable for $18.9K in total, and the best take nets $0.04 before $0.67 of gas. BSC: 20 fillable, best $0.10. Base: two, dust. Bots (`0xc0ffeebabe…`, `0xf4d0…`, `0x528a…`) and the resolvers already harvest the drift: ungated makers give away 16 bps at fill on average, 28% of ungated Ethereum volume fills more than 20 bps off the market, on strategies a median of 4.5 hours old.
- The incumbents stay fresh by re-shipping, not by reading prices: 94,797 ships for 457K fills over 42 days on the RES-gated concentrated template, 2,200 ships a day, about 0.18 ETH of gas a day at 1 gwei, and still 2.6 bps given at fill and 2.2 bps lost at five minutes. An oracle-anchored program reads the pool at every quote and never re-ships; that is the structural difference to show, not an arbitrage.
...
- A guard that is right on one source is wrong on another. Removing the maker's own prints from a reference window is correct on the tape; applied to a pool leg it subtracts tape quantities from pool sums and the leftover ratio is garbage (a DAI/USDT reference of 0.966, an ONDO one 87% off). Every guard names the source it applies to. A thin pool's own hourly median is no yardstick either: a lone print is its own median. The hourly dollar ratio is.
...
- Probe inside the cap. `RiskCapExceeded` is `0x93700de9`; quoting 1 WETH against a 1 WETH / 2,500 USDC ledger under a 20% cap reverts on the cap, not on the program. The validator probes at a twentieth of the ledger; a cap tighter than that is itself a rejection.
- Never `evm_snapshot`/`evm_revert` the gym while graph-node and the engine run on it: the revert replaces mined blocks, graph-node sees a reorg, Aquascan keeps fills that no longer exist. Validate on a private `anvil --fork-url <gym>`; it starts in under a second and forks the gym at its head.
- `currentGeneration()` reverts with `NoOpenGeneration()` when the last generation is closed; read it with a catch. A run that dies after opening leaves the generation open; the loop reuses an open generation rather than opening another (`GenerationStillOpen`).
- Aquascan's numbers for a shipped strategy keep moving after the generation closes; the attested `Score` is the record. Rank from the on-chain champion, not from the API's current figures.
- Aqua's `dock(address app, bytes32 strategyHash, address[] tokens)` takes the hash, not the strategy bytes; the tokens never left the wallet, so docking moves nothing.
- The repo `.env` ships `ANTHROPIC_API_KEY=` empty; the minds need a key from Raouf. The control line runs without one.
...
- Published DEX subgraphs are uneven. On the network, "Uniswap V3 Ethereum" (5zvR82…) and "Uniswap V3 BNB" (G5MUbS…) answer the pools/swaps/poolDayData schema through the gateway; the Arbitrum and Optimism entries with the most signal use a different schema (no `pools`), the Base and Polygon ones returned bad-indexer errors on the day, and the PancakeSwap BSC entries do not expose `pools`. Probe the schema and the indexer before routing anything through a subgraph, and keep the hourly fallback.
