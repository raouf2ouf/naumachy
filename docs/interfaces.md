...
- `GET /api/strategy/:hash` decoded program (sentence, pipeline, raw hex), lifecycle, P&L, fills with implied prices, and `card`: the program read back instruction by instruction (`aquascan/api/src/swapvm.ts`, layouts from SwapVM v1.0.2), each with `role` (gate, branch, fee, curve, balances, guard, clock, salt), a sentence, decoded `fields`, jump `target` and `landing`, `wraps` for middleware; plus `curve` for the XYC, concentrated and limit families: base and quote, spot, range, declared depth, and the price a taker gets for trades of 1K to 10M quote units in both directions. Declared balances are virtual depth, not inventory; Aqua caps a fill at the wallet.
...
- `GET /api/maker/:chain/:address` the maker in full, with `pnl` (realised and unrealised by average cost since first seen, coverage as fills priced against unpriced, oldest mark hour) and `pnl_tokens` (per token: position, average cost, latest price, realised, unrealised, legs; from `maker_pnl`), and `rewards` (null until Merkl has been read for the wallet; else `checked_at`, `total_usd`, and per reward token amount, claimed, pending, usd, campaign entries; from `maker_rewards`, wallet-level, all networks). Identity: realised + unrealised = `pnl_usd_marked` when every fill is priced, because each fill is valued self-financing (the best-priced leg at the hour's USD price, the other at the fill's own rate). Also the row above with up to six pairs, its templates with their scored numbers, its strategies newest first (with the template name, paged by `offset` and `limit`), its fee summary and its recent economic fills.
