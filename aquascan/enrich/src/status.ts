...
export interface ChainStatus {
  ...
  priceHours: number; priceMisses: number; tokens: number;
}
...
export async function status(pool: Pool): Promise<ChainStatus[]> {
  ...
  return rows.map((r) => ({
    ...
    priceHours: Number(r.price_hours), priceMisses: Number(r.price_misses), tokens: Number(r.tokens),
  }));
}

export function formatStatus(rows: ChainStatus[]): string {
  const head = `${"chain".padEnd(9)} ${"strategies".padStart(10)} ${"live".padStart(6)} ${"fills".padStart(8)} ${"economic".padStart(8)} ${"legs".padStart(8)} ${"cursor".padStart(12)} ${"head".padStart(12)} ${"behind".padStart(8)} ${"calls".padStart(6)} ${"prices".padStart(8)} ${"misses".padStart(7)} ${"tokens".padStart(7)}`;
